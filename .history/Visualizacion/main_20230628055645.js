...
// Call the loadData function and handle the data
loadData()
  .then((data) => {
    // Data loading success
    VGSalesData = data;
    // Call the function that depends on VGSalesData
    getTopGenresByRegion("Other_Sales");
  })
  .catch((error) => {
    // Data loading error
...
function createPieChart(genres, region) {
  ...
  if (!pieChartSVG) {
    ...
    pieChartSVG = SVG2.append("g")
      .attr("transform", `translate(${WIDTH_2 / 2 }, ${HEIGHT_2 / 2 - 40})`);
  } else {
  ...
  pieChartSVG.append("text")
    ...
    .text(`Top selled genres in ${regionName}`)
