...
let mouseOnCountry; // Variable para almacenar el país con el mouse encima
let clickedCountry; // Variable para almacenar el país seleccionado
  ...
  function ready(error, worldData) {
    ...
    contenedorPrimeraVisualizacion
      ...
      .on("mouseover", (event, d) => {
        mouseOnCountry = d3.select(event.currentTarget);
        ...
        mouseOnCountry.classed("highlighted", true);
      })
      .on("mouseout", (event, d) => {
        const countryName = d.properties.name;
        ...
        mouseOnCountry.classed("highlighted", false);
      })
      .on("click", (event, d) => {
        clickedCountry = d3.select(event.currentTarget);
        const countryName = d.properties.name;
        ...
        mouseOnCountry.classed("selected", true);
      ...
      // Remover la clase de selección de todos los países
      contenedorPrimeraVisualizacion.selectAll(".country").classed("selected", false);

      // Aplicar la clase de selección al país clickeado
