      ...
      .on("click", (event, d) => {
        ...
        contenedorPrimeraVisualizacion
          ...
          .filter((d) => {
            ...
            return !excludedCountriesEurope.includes(d.properties.name) &&
                    !excludedCountriesUSA.includes(d.properties.name) &&
                    !excludesCountriesJapan.includes(d.properties.name);
          })
          .classed("highlighted-region", function(d) {
            // Check if the country has the "highlighted-region" class
            const hasHighlightedClass = d3.select(this).classed("highlighted-region");
          
            // Add your condition to determine if the country should be highlighted
            if (hasHighlightedClass) {
              console.log("Country already highlighted");
              return false; // Highlight the country
            }
            return true; // Do not highlight the country
          });
