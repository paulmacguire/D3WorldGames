...
let selectedRegions = new Set(); // Conjunto para almacenar las regiones seleccionadas
  ...
  function ready(error, worldData) {
    ...
    contenedorPrimeraVisualizacion
      ...
      .on("click", (event, d) => {
        ...
        // Agregar o eliminar la región seleccionada del conjunto
        ...
        // Resaltar los países de las regiones seleccionadas en rojo
        contenedorPrimeraVisualizacion
          .selectAll(".country")
          .filter((d) => {
            const regionCountryName = d.properties.name;
            if (selectedRegions.has("NA_Sales")) {
              return excludedCountriesUSA.includes(regionCountryName);
            } else if (selectedRegions.has("EU_Sales")) {
              return excludedCountriesEurope.includes(regionCountryName);
            } else if (selectedRegions.has("JP_Sales")) {
        ...
        // Obtener los juegos más vendidos en las regiones seleccionadas
        const topGames = [];
        selectedRegions.forEach((region) => {
          const games = getTopGamesByRegion(region);
          topGames.push(...games);
        });
