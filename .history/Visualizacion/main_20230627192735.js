...
function createPieChart(genres) {
  ...
  arcs.append("text")
    .attr("transform", function (d) {
      ...
      const nx = (x / h) * (radius + 10); // New X position with an offset of 10 pixels
      const ny = (y / h) * (radius + 10); // New Y position with an offset of 10 pixels
      return `translate(${nx}, ${ny})`; // Translate the text element to the new position
    })
    .text(function (d) {
      return d.data.name;
    })
