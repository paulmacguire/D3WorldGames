...
const WIDTH_3 = 1000 - MARGIN.left - MARGIN.right;
const HEIGHT_3 = 800 - MARGIN.top - MARGIN.bottom;
...
function createVGMap() {
  ...
  function ready(error, worldData) {
    ...
    function createPieChart(genres) {
      const radius = 150; // Reduce the radius to fit the pie chart within the SVG element
      const centerX = radius + 10; // Adjust the X position to center the pie chart
      const centerY = radius + 10; // Adjust the Y position to center the pie chart
      ...
      arc.append("path")
        .attr("transform", `translate(${centerX}, ${centerY})`) // Translate the pie chart to the center
        .attr("d", path)
      ...
      arc.append("text")
        .attr("transform", function (d) {
          return `translate(${label.centroid(d)[0] + centerX}, ${label.centroid(d)[1] + centerY})`; // Translate the labels relative to the center
        })
        .text(function (d) {
          return d.data.name;
        })
