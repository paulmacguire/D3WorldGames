...
const WIDTH_1 = 3000 - MARGIN.left - MARGIN.right;
  ...
  function ready(error, worldData) {
    ...
    contenedorPrimeraVisualizacion
      ...
      .on("click", (event, d) => {
        ...
        const countryIndex = selectedCountries.indexOf(countryName);
        if (countryIndex === -1) {
          // El país no está seleccionado, agregarlo a la lista
          selectedCountries.push(countryName);
        } else {
          // El país ya está seleccionado, eliminarlo de la lista
          selectedCountries.splice(countryIndex, 1);
        }

        // Actualizar la clase CSS del país seleccionado
        clickedCountry.classed("selected", countryIndex === -1);
