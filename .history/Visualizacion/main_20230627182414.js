...
function createVGMap() {
  ...
  function ready(error, worldData) {
    ...
    function getTopGenresByRegion(regionsArray) {
      // Filtrar los juegos por las regiones seleccionadas y obtener la cantidad de ventas por género de juego
      console.log(regionsArray);
      const games = VGSalesData.filter((game) => { 
        return regionsArray.includes(game.Region) && game.Genre != "Misc"
      });
      const genres = {};
    ...
    contenedorPrimeraVisualizacion
      ...
      .on("click", (event, d) => {
        ...
        const topGames = getTopGamesByRegion(highlightedRegions);
