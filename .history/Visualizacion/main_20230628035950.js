...
function createPieChart(genres, region) {
  ...
  if (!pieChartSVG) {
    ...
    // Initialize pie, arc, path, label, and title variables
    pie = d3.pie().value(function(d) {
      return d.sales;
    });

    arc = pieChartSVG.selectAll("arc");
    ...
    pieChartTitle = pieChartSVG.append("text")
      .attr("x", 0)
      .attr("y", -radius - 10)
  ...
  pieChartTitle.text(`${region} Sales`);

  var ordScale = d3.scaleOrdinal()
    .domain(genres.map((d) => d.name))
    .range(colorPalettes[region]);

  var arcs = arc.data(pie(genres));
  ...
  arcs.selectAll("path")
    ...
    .on("mouseout", function(d) {
      ...
      // Restore the original size of the element
      currentElement
        .transition()
        .duration(200)
        .attr("d", d3.arc()
      ...
      // Restore the original appearance of the other elements
      arcs.selectAll("path")
        .filter(function() { return this !== currentElement.node(); })
        .transition()
        .duration(200)
